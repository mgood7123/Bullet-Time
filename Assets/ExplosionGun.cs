...
public class ExplosionGun : MonoBehaviour {
	...
	void Shoot ()
	{
		...
		if (Physics.Raycast(cam.transform.position, cam.transform.forward, out _hitInfo)) {
			// Create an explosion at the impact point, then delete the object
			Destroy((GameObject)Instantiate(explosion, _hitInfo.point, Quaternion.LookRotation(_hitInfo.normal)), 4);
		}
	}
